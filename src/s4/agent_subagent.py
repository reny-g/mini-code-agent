...
SYSTEM = f"You are a coding agent at {Path.cwd().parent}. Use the task tool to delegate exploration or subtasks."
SUBAGENT_SYSTEM = f"You are a coding subagent at {Path.cwd().parent}. Complete the given task, then summarize your findings."
...
def run_subagent(prompt: str) -> str:
    ...
    for _ in range(30):
        ...
        for tool_call in msg.tool_calls:
            ...
            print(f"子agent调用工具：{name}-{arguments}")
        
    # 返回子agent的最终响应
    return "".join(
        msg.content if not msg.content else "no summary."
    )
...
def agent_loop(messages: list):
    ...
    while True:
        ...
        # 如果有 tool_calls，检查是否为 todo 工具调用
        use_todo = False
        for tool_call in msg.tool_calls:
